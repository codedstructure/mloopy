...
class MidiProcess(threading.Thread):
    ...
    def processMsg(self):
        ...
        if 0x80 <= self.msg[0] <= 0x9F:
            ...
            # all on middle C
            #if self.msg[1] < 0x15+16:
            #    self.msg[0] |= (self.msg[1] - 0x15)
            #    self.msg[1] = 64 # middle C?
            # scale velocity
            self.msg[2] **= self.volmul
            self.msg[2] *= ((1+self.volslope)-self.msg[1]/64.0)
            self.msg[2] = max(0, min(127, self.msg[2]))
